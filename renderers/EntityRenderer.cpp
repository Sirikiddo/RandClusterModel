...
void EntityRenderer::renderEntities(const HexSphereRenderer::RenderContext& ctx) const {
    ...
    // Базовый размер сущности (для L=2)
    const float baseEntitySize = 0.08f;

    // Масштабируем в зависимости от размера ячейки
    const float entityScale = baseEntitySize * ctx.cellSize;
