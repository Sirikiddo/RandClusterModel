...
// Также нужно вызывать updateTreeOccupiedCells при перестройке модели:
void HexSphereSceneController::rebuildModel() {
    ico_ = icoBuilder_.build(L_);
    model_.rebuildFromIcosphere(ico_);
    regenerateTerrain();
    updateTreeOccupiedCells(); // <-- Добавить здесь
}
...
void HexSphereSceneController::regenerateTerrain() {
    ...
    updateTerrainMesh();
    updateTreeOccupiedCells(); // <-- И здесь
}
...
float HexSphereSceneController::cellSize() const {
    // Эмпирическая формула: размер ячейки уменьшается с ростом L
    // Для L=2 (по умолчанию) возвращаем 1.0
    // Для L=0 (крупные ячейки) ~2.0
    // Для L=7 (мелкие ячейки) ~0.3
    const float baseForL2 = 1.0f;
    const float factor = 0.7f; // коэффициент уменьшения
...
void HexSphereSceneController::updateTreeOccupiedCells() {
    ...
    for (size_t i = 0; i < cells.size(); ++i) {
        // Логика такая же как в EntityRenderer::renderTrees
        if (cells[i].biome == Biome::Grass && (i % 3 == 0)) {
            treeOccupiedCells_.insert(static_cast<int>(i));
        }
    }
}
