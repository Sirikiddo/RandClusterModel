...
InputController::Response InputController::mousePress(QMouseEvent* e) {
    ...
    if (e->button() == Qt::LeftButton) {
        ...
        if (hit->isEntity) {
            // “ќЋ№ ќ сущность: выдел€ем еЄ и не трогаем €чейку
            selectEntity(hit->entityId, response);
            response.requestUpdate = true;
        }
        else if (hit->cellId >= 0) {
            // “ќЋ№ ќ €чейка: выдел€ем еЄ и возможно двигаем сущность
            scene_.toggleCellSelection(hit->cellId);
            uploadSelection();

            // ѕеремещаем выбранную сущность только если есть активна€ сущность
            // и пользователь кликнул по €чейке (не по сущности)
...
InputController::Response InputController::keyPress(QKeyEvent* e) {
    ...
    case Qt::Key_W:
    {
        ...
        // »щем первый свободный сосед
        int next = -1;
        for (int neighborId : c.neighbors) {
            if (neighborId >= 0 && !scene_.isCellOccupiedByTree(neighborId)) {
                next = neighborId;
                break;
            }
        }
        ...
        if (next < 0) {
            response.hudMessage = "No free neighboring cells available";
            return response;
        }

        // ќчищаем выделение €чеек перед перемещением
...
void InputController::moveSelectedEntityToCell(int cellId, Response& response) {
    ...
    // ѕровер€ем, не зан€та ли €чейка деревом
    if (scene_.isCellOccupiedByTree(cellId)) {
        response.hudMessage = "Cannot move here: cell occupied by tree";
        response.requestUpdate = true;
        return;
    }
