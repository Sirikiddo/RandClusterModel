...
Чтобы собрать проект Planet:

1. Установите Qt 6.8+ с поддержкой **Qt Widgets** и **Qt OpenGL**.
2. Откройте проект в Qt Creator или соберите через CMake (если добавлен CMakeLists.txt).
3. Убедитесь, что включена сборка ресурса `Planet.qrc`.
4. Соберите проект в режиме Debug/Release.
5. Запустите бинарный файл — он откроет главное окно с планетой.

**Где создаётся таймер игрового цикла:**
В `ui/HexSphereWidget` в конструкторе создаётся `QTimer`, который вызывает `advanceWaterTime(dt)` и `update()`, формируя периодический рендер-цикл.
...
* Папка: `scene/`
  ...
  * Используется для вычислительных задач (построение рёбер, триангуляция, контуры), тогда как объекты геймплея живут в `ECS/`.

Грубо говоря, `model/` описывает «чистую» топологию планеты (ячейки и их свойства), а `scene/` — производные структуры, необходимые для отображения и интерактивных операций (выделение, пути, контуры).
...
### 10. Где искать код для изменений
...
### 11. Типичные задачи и куда смотреть

Несколько типичных сценариев и точек входа в код:

* **Добавить новый тип сущности (юнит, маркер и т.п.)**

  * Создать нужные компоненты в `ECS/` (если их ещё нет).
  * В `InputController::initialize` или отдельном методе создать сущность через `ecs_.createEntity(...)` и навесить компоненты.
...
* **Изменить генерацию рельефа**
  ...
  * Добавить новый класс-генератор и зарегистрировать его в `HexSphereSceneController`.

* **Изменить визуальный стиль воды или террейна**

  * Найти соответствующий под-рендерер в `renderers/`.
...
* **Добавить новый компонент в ECS**
  ...
  * Зарегистрировать его в `ComponentStorage`.
...
* Основные типы шейдеров:
  ...
  * шейдеры террейна (terrain) — отвечают за отображение рельефа и биомов;
  * шейдеры воды (water) — добавляют анимацию и эффекты поверхности воды;
  * шейдеры сущностей (entities) — применяются к объектам из ECS;
  * шейдеры оверлея (overlay) — рисуют контуры выделения и пути.
...
* найдите соответствующий шейдер в `resources/` (файлы `.glsl`, `.vert`, `.frag`);
* измените код шейдера (цвета, освещение, туман, воду и т.п.);
* при необходимости обновите привязку uniform-параметров в рендерере;
* пересоберите проект, чтобы Qt заново упаковал ресурсы.
