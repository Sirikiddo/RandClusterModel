...
QString PerformanceStats::getDetailedStats() const {
    // Ðàñ÷åò ïðèìåðíîãî èñïîëüçîâàíèÿ ïàìÿòè
    size_t approxMemory = (currentVertices_ * 3 * sizeof(float)) + // ïîçèöèè
        (currentVertices_ * 3 * sizeof(float)) + // öâåòà  
        (currentVertices_ * 3 * sizeof(float)) + // íîðìàëè
        (currentIndices_ * sizeof(uint32_t));    // èíäåêñû
