...
public:
    ...
    void draw() const {
        ...
        // Заливка (материал)
        glColor3f(0.2f, 0.5f, 0.8f); // Цвет влияет на материал через GL_COLOR_MATERIAL
        glBegin(GL_TRIANGLES);
        for (size_t i = 0; i < indices.size(); i += 3) {
            Vector3 v1 = vertices[indices[i]];
            Vector3 v2 = vertices[indices[i + 1]];
            ...
            // Нормаль к вершине = нормализованные координаты вершины (так как сфера в центре (0,0,0))
...
// Создание сферы с радиусом 1.0 и 4 уровнями подразделения
GeodesicSphere sphere(1.0f, 4);
...
void display() {
    ...
    glEnable(GL_COLOR_MATERIAL); // Цвет вершин будет влиять на материал
    glColorMaterial(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE); // Настроить связь цвета и материала
    GLfloat lightPos[] = { 5.0f, 5.0f, 5.0f, 1.0f };
    glLightfv(GL_LIGHT0, GL_POSITION, lightPos);

    
    sphere.draw();
    // Цвета света: фоновый, диффузный, зеркальный
    ...
    // Свойства материала: зеркальность и блеск
